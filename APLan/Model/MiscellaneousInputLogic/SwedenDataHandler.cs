...
namespace C_sharp_learning
{
    public class SwedenDataHandler
    {
        ...
        public ERDMmodel getInfraStructure()
        {
            ...
            return this.erdmModel;
        }
        private void extractLinks(XmlNode nodes)
        {
            ...
            foreach (XmlNode item in nodes.ChildNodes)
            {
                ...
                for (int i=0; i<coordiantes.Count-1; i++)
                {
                    ...
                    addCurveSegmentLine(point1, point2, azimuth, UniversalId, erdmModel);
                }

            }
        }
        private TrackEdgePoint addTrackEdgePoint(GeoCoordinates coor, TrackEdge edge,double offset,ERDMmodel erdmModel)
        {
        ...
        {
            var TrackSection = erdmModel.Tier2.TrackEdgeSection?.Find(x => x.hasStartTrackEdgePoint != null && x.hasStartTrackEdgePoint.Equals(startPoint.id) && x.hasEndTrackEdgePoint != null && x.hasEndTrackEdgePoint.Equals(endPoint.id));
            var OldCurveSegment = erdmModel.Tier3.CurveSegmentLine?.Find(x => x.azimuthAngle != null && x.azimuthAngle==azimuth);

            if (OldCurveSegment!=null)
            {
                OldCurveSegment.appliesToTrackEdgeSection?.Add(TrackSection.id);
                return OldCurveSegment;
            }
        ...
        private List<string> getCoordiantes(XmlNode node)
        {
            ...
            return new() { northing, easting, heightOverSeaype };
        }
